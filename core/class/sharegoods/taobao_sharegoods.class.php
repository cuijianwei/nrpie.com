...
class taobao_sharegoods implements interface_sharegoods
{
	public function fetch($url)
	{
		...
		$req->setFields("detail_url,title,nick,pic_url,price,cid,seller_cids ");
		$req->setNumIid($id);
...
//		}
		if(!empty($goods['cid']))
		{
			...
			$req->setCids($goods['cid']);
			//$req->setParentCid($goods['cid']);
			$resp = $client->execute($req);
			if($resp->item_cats->item_cat->is_parent == 'false' && $resp->item_cats->item_cat->parent_cid != '0')
			{
				$req->setCids($resp->item_cats->item_cat->parent_cid);
				$resp = $client->execute($req);
			}
			$cate = (array)$resp->item_cats->item_cat;
			$result['item']['cateName'] = strpos($cate['name'],'/') ? substr($cate['name'], 0, strpos($cate['name'],'/')) : $cate['name'];
//			var_dump($resp);die;
		}
		return $result;
	}

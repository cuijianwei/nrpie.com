...
class ShareService
{
	...
	public function save($data,$is_score = true)
	{
		...
		if(intval($share_id)>0)
		{
			...
			//更新会员统计
			$condtions = $share_data_now_type == 'look' ? ',look=look+1' : ($share_data_now_type == 'dapei' ? ',dapei=dapei+1' : '');
			FDB::query('UPDATE '.FDB::table('user_count').'
				SET shares = shares + 1,goods = goods + '.$goods_count.',photos = photos + '.$photo_count.$condtions.'
	...
	public function deleteShare($share_id,$is_score = true)
	{
		...
		if(!empty($share))
		{
			...
			$condtions = $share['type'] == 'look' ? ',look=look-1' : ($share['type'] == 'dapei' ? ',dapei=dapei-1' : '');
			FDB::query('UPDATE '.FDB::table('user_count').' SET
				shares = shares - 1,
				photos = photos - '.$photo_count.',
				goods = goods - '.$goods_count.',
				collects = collects - '.$collect_count.$condtions.' WHERE uid = '.$share['uid']);

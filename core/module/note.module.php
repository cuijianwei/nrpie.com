...
class NoteModule
{
	...
	private function show($current_type = '')
	{
			...
			switch($current_type)
			{
				case 'bao':
					foreach($share_detail['imgs'] as $img)
					{
						if($img['type'] == 'g')
						{
							if(strlen($img['name']) > 18)
								$img['name'] = mb_substr($img['name'],0,18,'utf-8')."......";
							$all_obj[] = $img;
						}
		...
		$invite_types = array('renren','sina','tqq','qzone');
		...
		include template('page/note/note_index_new');
		display();
	}
}
?>
...
class UModule
{
	...
	public function topic()
	{
		...
		$pager = buildPage('u/'.ACTION_NAME,$page_args,$home_user['forums'],$_FANWE['page'],$page_size,'',3);
		//$pager = buildPage('u/'.ACTION_NAME,$page_args,$home_user['forums'],$_FANWE['page'],10);

		$thread_list = array();
		$res = FDB::query('SELECT ft.*,s.cache_data,s.click_count FROM '.FDB::table('forum_thread').' AS ft
		...
		while($thread = FDB::fetch($res))
		{
			...
			$thread['page_args2'] = array(
					'tid'=>$thread['tid'],
			);			$thread['count2'] = $thread['post_count'];			$thread['pager2'] = buildPage2('club/detail',$thread['page_args2'],$thread['count2'],1,$page_size,'',3);
	...
	public function fans()
	{
		...
		$home_uid = $_FANWE['home_uid'];
		$home_user = FS('User')->getUserById($home_uid);
		//echo $home_uid;echo '<br>';				//echo $_FANWE['uid'];		//echo "update ".FDB::table("user_notice")." set num=0 where `type`=1 and uid=".intval($_FANWE['uid']);		
		if($_FANWE['uid']==$home_uid)
		{
			//将我的粉丝信息统计设置为0
